...
服务端接收由客户端取得的 Entra 委托 Bearer Token。OAuth Discovery、Public Client Registration、MSAL Interactive Sign-in、S256 PKCE 和一次性 Local Authorization-code Exchange 已位于 `OAUTH_ENABLED` 后；在 Persistent Refresh Session 与真实 WorkBuddy 验收完成前，该开关保持默认关闭。草稿、OCR、共享邮箱、RBAC、限流以及其他 Microsoft 365 工作负载尚未实现。Mock Graph/OBO/OAuth 测试不代表真实租户已经验收。详见[实现复核状态](docs/zh-CN/review-status.md)、[运行时依赖审计](docs/zh-CN/runtime-dependency-audit.md)和 [WorkBuddy OAuth 指南](docs/zh-CN/workbuddy-oauth.md)。
